...
$(document).ready(function(){
	...
	// new try
	leftRecognizer = new Hammer.Pan({ event:"panleft",direction:Hammer.Direction_Left});
	rightRecognizer = new Hammer.Pan({ event:"panright",direction:Hammer.Direction_Right});
	endRecognizer = new Hammer.Pan({ event:"panend",direction:Hammer.Direction_ALL});
	tapRecognizer = new Hammer.Tap({ event: 'tap', taps: 1 });

	var first = new Hammer.Manager(myElement);
	// first.add(endRecognizer);
	first.add(leftRecognizer);
	...
	function panLeft(event){
		console.log(event.type);
		first.remove(leftRecognizer);
		// first.off(leftRecognizer);
		first.add(rightRecognizer);
		first.on("panright",callback2);
		// first.stop(leftRecognizer);
		// first.add(downRecognizer);
		// first.remove(leftRecognizer);
	...
	var callback2 = function(e) {
	    console.log(e.type);
	    first.off(rightRecognizer, callback2);
	};

	function panRight(event){
		console.log(event.type);
		console.log("removing down recognizer!");
		first.stop(rightRecognizer);
		first.remove(rightRecognizer);
		// first.off(rightRecognizer);
		first.add(tapRecognizer);
	...
	function emergency(event){
		console.log(event.type);
		first.remove(tapRecognizer);
		first.stop(tapRecognizer);
		// first.off(tapRecognizer);
		$.post('/alerts#create',
